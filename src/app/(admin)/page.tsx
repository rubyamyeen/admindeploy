...
export default async function DashboardPage() {
  ...
  return (
    <div className="space-y-8">
      {/* Hero Header */}
      <div className="bg-gradient-to-r from-violet-600 to-purple-600 rounded-2xl p-8">
        <h1 className="text-3xl font-bold text-white">Dashboard</h1>
        <p className="text-white/80 mt-2">Welcome to the AlmostCrack&apos;d Admin Panel</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
        <StatCard title="Total Profiles" value={data.profileCount} gradient="bg-gradient-to-br from-blue-500 to-blue-600" />
        <StatCard title="Total Images" value={data.imageCount} gradient="bg-gradient-to-br from-emerald-500 to-emerald-600" />
        <StatCard title="Total Captions" value={data.captionCount} gradient="bg-gradient-to-br from-violet-500 to-purple-600" />
        <StatCard title="Public Images" value={data.publicImageCount} gradient="bg-gradient-to-br from-orange-500 to-orange-600" />
        <StatCard title="Common Use Images" value={data.commonUseImageCount} gradient="bg-gradient-to-br from-teal-500 to-teal-600" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div key={image.id} className="px-6 py-4 hover:bg-slate-700/30 transition-colors">
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-slate-400">{formatDate(image.created_datetime_utc)}</p>
                    </div>
        ...
        <div className="bg-slate-800/50 backdrop-blur-xl rounded-xl border border-slate-700/50 overflow-hidden">
          <div className="px-6 py-4 border-b border-slate-700/50">
            <h2 className="text-lg font-semibold text-white">Recent Captions</h2>
          </div>
          <div className="divide-y divide-slate-700/50">
            {data.recentCaptions.length === 0 ? (
              <p className="px-6 py-4 text-slate-400">No captions yet</p>
            ) : (
              data.recentCaptions.map((caption) => (
                <div key={caption.id} className="px-6 py-4 hover:bg-slate-700/30 transition-colors">
